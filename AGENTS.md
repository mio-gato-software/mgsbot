...
bun run memory:stats   # Background usage, embedding cache hits, memory prompt size
```
...
- **Semantic Store** (`memory/semantic.json`): Global knowledge base of `SemanticFact` objects with 768-dim embeddings. Categories: "person", "group", "rule", "event". Facts have `importance`, `confidence` (decays 0.02/day; below 0.1 marks `archivedAt`, preserving relevant historical recall), and `subject`. Deduplication via cosine similarity at 0.85 threshold. Facts injected into a prompt are reinforced (small confidence bump, throttled to once/hour per fact via `lastRecalledAt`), so often-recalled facts fade more slowly — but retrieval is usefulness, not evidence: the bump is capped by `reinforcementCeiling()` (0.75, eroding 0.01/day as `lastConfirmed` ages) and never touches the decay clock, so a fact the bot only ever quotes back to itself still ages into archival. Only genuine reconfirmation through extraction (dedup merges or `confirmedFacts` with verified verbatim user evidence) moves `lastConfirmed` and lifts a fact above the ceiling. Facts can be marked `permanent: true` for immutable biographical data (birthplace, family, marriage) — these never decay and have no storage-count cap; retrieval selects up to 25 by relevance/participant names within the memory prompt budget. A daily janitor (`src/janitor.ts`, disable with `ENABLE_MEMORY_JANITOR=false`) reviews changed subjects with ≥6 active, non-archived facts via the background model (successful fingerprints persist in `janitor-reviews.json`) and soft-retires contradicted/duplicate facts (`supersededBy` + `validUntil`, never hard deletion).
...
- **Chapters** (`memory/chapters/<chat_id>.json`): Per-chat monthly narrative chapters built from episodes (id `chapter_<chatId>_<YYYY-MM>`, title, compact summary, importance 1–5). Keeps 18 chapters; retrieves two by explicit date/month/year or topic relevance, with recent chapters as fallback.
...
**Memory cost:** All memory prompt sections share a 12,000-character ceiling with per-section allowances. `src/memory/usage-metrics.ts` records counts and available provider usage by operation/model, including retries/fallbacks and embedding-cache hits. `bun run memory:stats` reads these records; unavailable token counts are not estimated.
